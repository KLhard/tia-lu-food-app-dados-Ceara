...
while True:
    print("\nMenu Principal:")
    print("1 - Gerenciar Itens do Cardápio")
    ...
    if escolha == "1":
                ...
                if not items:
                    ...
                    for it in items:
                        if it[0] == cod:
                            ...
                            nova_desc = input(f"Nova descrição (enter para manter): ").strip()
        ...
        while True:
            ...
            print("2 - Processar Pedidos Pendentes (aceitar/rejeitar fila)")
        ...
        if sold_codes:
            ...
            for i in range(len(sqtys)):
                ...
                temp_q = sqtys[i]; sqtys[i] = sqtys[max_i]; sqtys[max_i] = temp_q
                temp_c = scodes[i]; scodes[i] = scodes[max_i]; scodes[max_i] = temp_c
            print("Itens mais vendidos (ENTREGUE):")
            for i in range(len(scodes)):
                print(f" - {scodes[i][1]} (Código {scodes[i][0]}): {sqtys[i]} unidades")
        else:
            print("Nenhum item vendido (ENTREGUE) até agora.")
